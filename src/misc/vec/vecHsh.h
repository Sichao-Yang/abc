...
static inline unsigned *Hsh_IntData(Hsh_IntMan_t *p, int iData) {
  return (unsigned *)Vec_IntEntryP(p->vData, p->nSize * iData);
}
static inline Hsh_IntObj_t *Hsh_IntObj(Hsh_IntMan_t *p, int iObj) {
  return iObj == -1 ? NULL : (Hsh_IntObj_t *)Vec_WrdEntryP(p->vObjs, iObj);
}
...
static inline Hsh_VecObj_t *Hsh_VecObj(Hsh_VecMan_t *p, int i) {
  return i == -1 ? NULL
                 : (Hsh_VecObj_t *)Vec_IntEntryP(p->vData,
                                                 Vec_IntEntry(p->vMap, i));
}
...
static inline int *Hsh_Int1ManLookupInt(Hsh_Int1Man_t *p, int Data) {
  int Item, *pPlace = Vec_IntEntryP(
                p->vTable, Hsh_Int1ManHash(Data, Vec_IntSize(p->vTable)));
...
static inline int Hsh_Int1ManAdd(Hsh_Int1Man_t *p, int Data) {
  ...
  if (Vec_IntSize(p->vData) > Vec_IntSize(p->vTable)) {
    ...
    Vec_IntForEachEntry(p->vData, Entry, i) {
      pPlace = Vec_IntEntryP(p->vTable,
                             Hsh_Int1ManHash(Entry, Vec_IntSize(p->vTable)));
...
***********************************************************************/
static inline Hsh_IntMan_t *Hsh_IntManStart(Vec_Int_t *vData, int nSize,
                                            int nEntries) {
  Hsh_IntMan_t *p;
...
static inline int *Hsh_IntManLookup(Hsh_IntMan_t *p, unsigned *pData) {
  Hsh_IntObj_t *pObj;
  int *pPlace = Vec_IntEntryP(
      p->vTable, Hsh_IntManHash(pData, p->nSize, Vec_IntSize(p->vTable)));
  for (; (pObj = Hsh_IntObj(p, *pPlace)); pPlace = &pObj->iNext)
    if (!memcmp(pData, Hsh_IntData(p, pObj->iData),
                sizeof(int) * (size_t)p->nSize))
      return pPlace;
  assert(*pPlace == -1);
  return pPlace;
}
static inline int Hsh_IntManAdd(Hsh_IntMan_t *p, int iData) {
  ...
  if (Vec_WrdSize(p->vObjs) > Vec_IntSize(p->vTable)) {
    ...
    for (i = 0; i < Vec_WrdSize(p->vObjs); i++) {
      pPlace = Vec_IntEntryP(
          p->vTable, Hsh_IntManHash(Hsh_IntData(p, Hsh_IntObj(p, i)->iData),
                                    p->nSize, Vec_IntSize(p->vTable)));
...
static inline Vec_Int_t *Hsh_WrdManHashArray(Vec_Wrd_t *vDataW, int nSize) {
  Hsh_IntMan_t *p;
  Vec_Int_t Data = {2 * Vec_WrdCap(vDataW), 2 * Vec_WrdSize(vDataW),
...
static inline Hsh_IntMan_t *Hsh_WrdManHashArrayStart(Vec_Wrd_t *vDataW,
                                                     int nSize) {
  Hsh_IntMan_t *p;
  int i, nEntries = Vec_WrdSize(vDataW) / nSize;
  Vec_Int_t *vData = Vec_IntAlloc(2 * Vec_WrdSize(vDataW));
  memcpy(Vec_IntArray(vData), Vec_WrdArray(vDataW),
         sizeof(word) * (size_t)Vec_WrdSize(vDataW));
  vData->nSize = 2 * Vec_WrdSize(vDataW);
  /*
      for ( i = 0; i < 30; i++ )
      {
          extern void Extra_PrintHex( FILE * pFile, unsigned * pTruth, int nVars
     ); Extra_PrintHex( stdout, (unsigned *) Vec_WrdEntryP(vDataW, i), 6 );
     printf( "  " ); Kit_DsdPrintFromTruth( (unsigned *) Vec_WrdEntryP(vDataW,
     i), 6 );   printf( "\n" );
      }
  */
  assert(Vec_IntSize(vData) % (2 * nSize) == 0);
...
static inline int Hsh_VecSize(Hsh_VecMan_t *p) { return Vec_IntSize(p->vMap); }
static inline double Hsh_VecManMemory(Hsh_VecMan_t *p) {
  return !p ? 0.0
            : Vec_IntMemory(p->vTable) + Vec_IntMemory(p->vData) +
                  Vec_IntMemory(p->vMap);
}
...
static inline int Hsh_VecManHash(Vec_Int_t *vVec, int nTableSize) {
  ...
  unsigned Key = 0;
  int i, Entry;
  Vec_IntForEachEntry(vVec, Entry, i) Key += (unsigned)Entry * s_Primes[i % 7];
  return (int)(Key % nTableSize);
}
static inline int Hsh_VecManAdd(Hsh_VecMan_t *p, Vec_Int_t *vVec) {
  ...
  if (Vec_IntSize(p->vMap) > Vec_IntSize(p->vTable)) {
    ...
    for (i = 0; i < Vec_IntSize(p->vMap); i++) {
      pPlace = Vec_IntEntryP(p->vTable, Hsh_VecManHash(Hsh_VecReadEntry(p, i),
                                                       Vec_IntSize(p->vTable)));
      Hsh_VecObj(p, i)->iNext = *pPlace;
      *pPlace = i;
    }
  }
  pPlace =
      Vec_IntEntryP(p->vTable, Hsh_VecManHash(vVec, Vec_IntSize(p->vTable)));
  for (; (pObj = Hsh_VecObj(p, *pPlace)); pPlace = &pObj->iNext)
    if (pObj->nSize == Vec_IntSize(vVec) &&
        !memcmp(pObj->pArray, Vec_IntArray(vVec),
                sizeof(int) * (size_t)pObj->nSize))
      return *pPlace;
  *pPlace = Vec_IntSize(p->vMap);
  ...
  Vec_IntForEachEntry(vVec, Ent, i) Vec_IntPush(p->vData, Ent);
